...
public class MockTestServiceImpl implements MockTestService {
    ...
    private final StudentMockTestAssignmentRepository studentMockTestAssignmentRepository;
    ...
    public MockTestResponse createMockTest(MockTestRequest request) {
        ...
        mockTest.setActive(request.isActive());
    ...
    public List<MockTestResponse> getAllMockTests() {
        ...
        for (MockTest mockTest : mockTestRepository.findAll()) {
            responses.add(mapToResponse(mockTest));
        }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MockTestResponse> getAvailableMockTests(
            String username) {
        StudentProfile student = getStudent(username);

        List<MockTestResponse> responses =
                new ArrayList<>();

        List<StudentMockTestAssignment> assignments =
                studentMockTestAssignmentRepository
                        .findByStudentProfileIdAndActiveTrueOrderByAssignedAtDesc(
                                student.getId()
                        );

        Set<Long> addedTestIds = new HashSet<>();

        for (StudentMockTestAssignment assignment : assignments) {
            MockTest mockTest = assignment.getMockTest();

            if (mockTest == null || !mockTest.isActive()) {
                continue;
            }

            if (!isPersonalizedMockTest(mockTest.getId())) {
                continue;
            }

            responses.add(
                    mapToAssignedPersonalizedResponse(assignment)
            );

            addedTestIds.add(mockTest.getId());
        }
        ...
        for (MockTest mockTest : mockTestRepository.findAll()) {
            ...
            if (addedTestIds.contains(mockTest.getId())) {
                continue;
            }
    ...
    public StartMockTestResponse startMockTest(
        ...
        MockTest mockTest =
                mockTestRepository
                        .findById(request.getMockTestId())
                        .orElseThrow(() ->
                                error(
        ...
        boolean personalized =
                isPersonalizedMockTest(mockTest.getId());
        ...
        if (personalized) {
            StudentMockTestAssignment assignment =
                    getLockedPersonalizedAssignment(
                            student,
                            mockTest
                    );
            ...
            testAttemptRepository
                    ...
                    .ifPresent(existing -> {
                        throw error(
                                HttpStatus.CONFLICT,
                                "An active attempt already exists for this mock test."
                        );
                    });
            ...
            if (completedBefore) {
                if (assignment.getAvailableRetakeCredits() <= 0) {
                    throw error(
                            HttpStatus.FORBIDDEN,
                            "You have already completed this mock test. Faculty permission is required for a retake."
                    );
                }

                assignment.consumeOneRetake();

                studentMockTestAssignmentRepository.save(
                        assignment
                );
            }
            ...
            chosen =
                    mappings.stream()
                            .map(MockTestQuestion::getQuestion)
                            .toList();

            attemptSkillLabel =
                    buildPersonalizedSkillLabel(mappings);

        } else {
            if (request.getSelectedSkill() == null ||
                    request.getSelectedSkill().isBlank()) {
                throw error(
                        HttpStatus.BAD_REQUEST,
            ...
            attemptSkillLabel = selectedSkill;
        }
    ...
    private StartMockTestResponse createAttempt(
        ...
        attempt.setSelectedSkill(skillLabel);
        attempt.setTestTitle(mockTest.getTitle());
        ...
        attempt.setExpiresAt(
                attempt.getStartedAt().plusMinutes(
                        mockTest.getDurationMinutes()
                )
        );
        ...
        testAttemptQuestionRepository.saveAll(snapshots);
    ...
    private List<Question> selectManualQuestions(
        ...
        chosen.addAll(
                selectQuestions(
                        questionRepository.findByCategory(
    ...
    private StudentMockTestAssignment
    getLockedPersonalizedAssignment(
            StudentProfile student,
            MockTest mockTest) {
        return studentMockTestAssignmentRepository
    ...
    loadAndValidatePersonalizedMappings(
        ...
        if (mappings.size() != AI_TOTAL_COUNT) {
            throw error(
                    HttpStatus.CONFLICT,
        ...
        for (MockTestQuestion mapping : mappings) {
            ...
            if (mapping.getQuestionOrder() == null ||
                    mapping.getQuestionOrder() != expectedOrder) {
                throw error(
                        HttpStatus.CONFLICT,
            ...
            if (question.getCategory()
                ...
                if (question.getTechnicalSkill() == null ||
                        question.getTechnicalSkill().isBlank()) {
                    throw error(
                            HttpStatus.CONFLICT,
    ...
    private String buildPersonalizedSkillLabel(
        ...
        for (MockTestQuestion mapping : mappings) {
            ...
            String skill = question.getTechnicalSkill();
        ...
        String label = String.join(", ", skills);
    ...
    public SubmitMockTestResponse submitMockTest(
        ...
        requireAttemptMetadata(attempt);

        if (attempt.getMockTestResult() != null) {
            return mapToSubmitResponse(attempt);
        }
        ...
        for (TestAttemptQuestion question : snapshots) {
            if (updates.containsKey(
                    question.getQuestionId())) {
                question.setSelectedOption(
                        updates.get(question.getQuestionId())
                );
            }
        }
    ...
    public StudentQuestionResponse saveAnswer(
        ...
        testAttemptQuestionRepository.save(question);

        return mapToStudentQuestion(question);
    }
    ...
    public boolean finalizeExpiredAttempt(
        ...
        TestAttempt attempt =
                testAttemptRepository
                        .findByIdForUpdate(testAttemptId)
                        .orElse(null);
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MockTestRetakeResponse>
    getRetakeStatuses() {
        List<MockTestRetakeResponse> responses =
                new ArrayList<>();

        for (StudentMockTestAssignment assignment
                : studentMockTestAssignmentRepository.findAll()) {
            if (assignment.getStudentProfile() == null ||
                    assignment.getMockTest() == null) {
                continue;
            }
    ...
    @Override
    public MockTestRetakeResponse allowRetake(
            Long assignmentId) {
        requireId(
                assignmentId,
                "Assignment ID"
        ...
        if (!Boolean.TRUE.equals(
            ...
            throw error(
                    HttpStatus.CONFLICT,
                    "Mock test assignment is not active."
            );
        }

        StudentProfile student =
                assignment.getStudentProfile();

        MockTest mockTest =
                assignment.getMockTest();

        if (student == null || mockTest == null) {
            throw error(
                    HttpStatus.CONFLICT,
                    "Invalid mock test assignment."
            );
        }
        ...
        if (completedAttemptCount == 0) {
            throw error(
                    HttpStatus.CONFLICT,
                    "The student has not completed the first attempt."
            );
        }

        if (assignment.getAvailableRetakeCredits() == 0) {
            assignment.grantOneRetake();
        ...
        return mapToRetakeResponse(
                assignment,
                completedAttemptCount
        );
    }

    private MockTestRetakeResponse mapToRetakeResponse(
            StudentMockTestAssignment assignment,
            long completedAttemptCount) {
        StudentProfile student =
                assignment.getStudentProfile();
        ...
        int retakeCredits =
                assignment.getAvailableRetakeCredits();
        ...
        } else if (retakeCredits > 0) {
            status = "RETAKE_ALLOWED";

        } else {
            status = "FACULTY_PERMISSION_REQUIRED";
        }

        MockTestRetakeResponse response =
                new MockTestRetakeResponse();
        ...
        response.setMockTestTitle(
                mockTest.getTitle()
        );
        response.setCompletedAttemptCount(
                completedAttemptCount
        );
        response.setRetakeCredits(
                retakeCredits
        );
        response.setAttemptAllowed(
                attemptAllowed
        );
        response.setAssignmentActive(
                Boolean.TRUE.equals(
                        assignment.getActive()
                )
        );
        ...
        response.setLastRetakeGrantedAt(
                assignment.getLastRetakeGrantedAt()
        );
        response.setLastRetakeConsumedAt(
                assignment.getLastRetakeConsumedAt()
        );
    ...
    private void requireAttemptMetadata(
        ...
        if (attempt.getTotalQuestions() == null ||
                ...
                attempt.getPassPercentage() == null ||
                attempt.getPassPercentage() < 0 ||
                attempt.getPassPercentage() > 100 ||
                attempt.getStartedAt() == null ||
                attempt.getExpiresAt() == null ||
                attempt.getDurationMinutes() == null ||
                attempt.getDurationMinutes() <= 0 ||
                attempt.getSelectedSkill() == null ||
                attempt.getTestTitle() == null) {
            throw error(
                    HttpStatus.CONFLICT,
    ...
    private List<TestAttemptQuestion> getSnapshots(
            TestAttempt attempt) {
    ...
    private Map<Long, String> validateAnswers(
        ...
        if (answers.size() > snapshots.size()) {
            throw error(
                    HttpStatus.BAD_REQUEST,
        ...
        for (TestAttemptQuestion question : snapshots) {
            assignedIds.add(question.getQuestionId());
        }
    ...
    private SubmitMockTestResponse finishAttempt(
        ...
        for (TestAttemptQuestion question : questions) {
            if (question.getSelectedOption() != null &&
                    question.getSelectedOption().equals(
        ...
        boolean passed =
                (long) correct * 100
                        >=
                        (long) attempt.getPassPercentage()
                                * total;
        ...
        result.setAttemptedAt(submittedAt);

        result =
                mockTestResultRepository.save(result);

        attempt.setMockTestResult(result);
        attempt.setCompleted(true);
        attempt.setAutoSubmitted(
                automatic || attempt.isAutoSubmitted()
        );
    ...
    private TestAttemptQuestion snapshot(
            ...
            Question source,
            int position) {
        String correct = source.getCorrectOption();
        ...
        question.setTestAttempt(attempt);
        question.setQuestionId(source.getId());
        question.setQuestionNumber(position);
    ...
    private boolean isPersonalizedMockTest(
        ...
        return mockTestQuestionRepository
                .existsByMockTestId(mockTestId);
    }

    private StartMockTestResponse mapToStartResponse(
            TestAttempt attempt,
            List<TestAttemptQuestion> questions) {
    ...
    private StudentQuestionResponse mapToStudentQuestion(
            TestAttemptQuestion question) {
    ...
    private SubmitMockTestResponse mapToSubmitResponse(
            TestAttempt attempt) {
        MockTestResult result =
    ...
    private MockTestResponse mapToResponse(
        ...
        if (isPersonalizedMockTest(
            ...
            response.setSelectedSkills(
                    extractTechnicalSkills(mappings)
            );
